          ...
          <div class="bg-base-200 rounded p-4 pb-0 mt-5 hidden md:block">
            ...
            <div v-if="anime.info.totalEpisodes > 25" class="pb-8">
              <label class="form-control w-full max-w-xs">
                ...
                <select class="select select-bordered w-full max-w-xs" v-model="episode">
                  <option v-for="ep in anime.info.totalEpisodes" :key="ep" :value="ep" :selected="episode == ep">{{ ep }}</option>
                </select>
              </label>
            </div>
          </div>
...
<script setup lang="ts">
  ...
  const { data: anime, status, error } = await useAsyncData(`anime:${animeId}`, async () => {
    const [info, stream] = await Promise.all([
      ...
      $fetch(`${config.public.apiBase}/meta/anilist/watch/${animeId}-episode-${episode.value}`)
    ])
  ...
  const updateServerUrl = () => {
    if (anime.value && anime.value.stream && anime.value.stream.sources.length > 0) {
      const streamIndex = anime.value.stream.sources.findIndex(source => source.quality === 'default')
      serverIndex.value = streamIndex !== -1 ? streamIndex : 0
      serverUrl.value = anime.value.stream.sources[serverIndex.value]?.url || ''
    }
  }
  
  watch(() => anime.value, (newAnime) => {
    if (newAnime) {
      updateServerUrl()
      
      useHead({
        title: `${newAnime.info.title} | Episode ${episode.value}`
      })
  
      $watchHistory.storeNewHistory({
        id: newAnime.info.id,
        title: newAnime.info.title,
        image: newAnime.info.image,
        ep: episode.value,
      })
    }
  }, { immediate: true })
  ...
  function changeServer(index: number) {
    ...
    serverUrl.value = anime.value?.stream.sources[index]?.url || ''
  }
  
  watch(episode, async (newEpisode, oldEpisode) => {
    await navigateTo(`/anime/${animeId}/watch/${newEpisode}`)
  })
  
  watch(serverIndex, (newServer) => {
